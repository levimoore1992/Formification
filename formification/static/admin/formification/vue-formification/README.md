# vue-formification — Vue 3 admin for the Formification Django app
...
```sh
# Dev — hot reload, proxies /formification + /admin to Django on :8000
npm install
...
```
vue-formification/
├── index.html              # dev shell; the <div id="formification-container"> mount
...
The Vue app is mounted at `/admin/formification/form/:id/change/`, served by
`formification/admin.py` (`FormAdmin.changeform_view`) and
`formification/templates/admin/formification/form/change_form.html`:

1. **Container** — mounts at the existing `<div id="formification-container"></div>`.
2. **Bootstrap config** — `changeform_view` injects `vue_config`
   (`{"formId": <pk>, "apiBase": "/formification/api"}`) into context; the template
   emits `<meta name="vue-formification/config/environment" content="{{ vue_config }}">`.
3. **Assets** — the length-one `{% vue_formification_assets %}` tag (setting_tags.py)
...
The TinyMCE Cloud script (`{% formification_tinymce_key %}`) is in the template and
...
the app's static dir (`formification/static/.../vue-formification/dist/`) via
...
DRF viewsets from `formification/urls.py` (router = `/formification/api/`, trailing
...
(change_formification_form-style perms). CSRF forwarded from the `csrftoken` cookie
...
- [x] Wire into `change_form.html` (Vue build served via `vue_formification_assets`)
...
- **CSV export is outside the SPA** — the "Download CSV" link targets Django's
  `/formification/download/submissions/?form=:id` (`views.download_submissions`,
...
- The public-facing form (`formification/static/formification/vite-formification/src/custom_form.js`,
