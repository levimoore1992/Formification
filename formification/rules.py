...
class Field(object):
    ...
    def has_value(self, value):
        ...
        if type(self.form_field) is BooleanField:
            return self.value
        elif type(self.value) is list:
            return any(self._equal(value, item) for item in self.value)
        else:
            return self._equal(self.value, value)

    @staticmethod
    def _equal(a, b):
        if a == b:
            return True
        # Choice fields post option ids as strings; conditions may store ids
        # as ints (or vice versa), so compare loosely like the client-side JS.
